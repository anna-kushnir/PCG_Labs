...
public class CylinderMode extends WorkMode {
    ...
    public CylinderMode() {
        ...
        cameraCoords = new float[]{ 0.0f, -4.0f, 2.5f };
        lightCoords = new float[]{ -0.3f, -0.7f, 0.5f };
    }
    ...
    protected void createScene() {
        ...
        arrayVertex = new float[6 + 2 * numVertexCylinder * 6];

        int pos = GraphicPrimitives.addVertexXYZn(arrayVertex, 0, 0, 0, 0, 1, 1, 1);

        float outerCylinderRadius = 0.5f,
                innerCylinderRadius = 0.499f,
                cylinderHeight = 1.0f,
                z = 0.0f;
        pos = GraphicPrimitives.addCylinderStrip(arrayVertex, pos, n, z, outerCylinderRadius, cylinderHeight, 1);
        GraphicPrimitives.addCylinderStrip(arrayVertex, pos, n, z, innerCylinderRadius, cylinderHeight, -1);
    }
    ...
    public void useProgramForDrawing(int width, int height) {
        ...
        // Малювання циліндрів (зовнішнього та внутрішнього)
        GLES32.glUniform3fv(vMatrixHandle, 1, cylinderColor, 0);
        GLES32.glBindVertexArray(VAO_id);
        GLES32.glDrawArrays(GLES32.GL_TRIANGLE_STRIP, 1, numVertexCylinder);
        GLES32.glDrawArrays(GLES32.GL_TRIANGLE_STRIP, 1 + numVertexCylinder, numVertexCylinder);
